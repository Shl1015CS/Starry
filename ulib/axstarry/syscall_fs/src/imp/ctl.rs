...
pub fn syscall_renameat2(
    ...
    // 旧文件是否存在
    match RenameFlags::from_bits(flags.try_into().unwrap()) {
        Some(flags) => {
            if flags.contains(RenameFlags::RENAME_EXCHANGE) {
                // 交换两个文件
                if path_exists(old_path.path()) && path_exists(new_path.path()) {
                    let _ = rename(old_path.path(), "/tmp/axfs_tmp");
                    let _ = rename(new_path.path(), old_path.path());
                    let _ = rename("/tmp/axfs_tmp", new_path.path());
                    return Ok(0);
                }
